...
aws cloudformation create-stack \
  --stack-name RomanNumeralsConverterApp \
  ...
  --parameters ParameterKey=InstanceCount,ParameterValue=1 ParameterKey=InstanceType,ParameterValue=t2.micro \
    ParameterKey=KeyPairName,ParameterValue=first-key ParameterKey=OperatingSystem,ParameterValue=Linux \
    ParameterKey=SSHLocation,ParameterValue=0.0.0.0/0 ParameterKey=TagKey,ParameterValue=Name \
    ParameterKey=TagValue,ParameterValue=MyApplication \
...
### 2. Call the describe-stacks command to verify the AWS CloudFormation stack named RomanNumeralsConverterApp was successfully created:

aws cloudformation describe-stacks --stack-name RomanNumeralsConverterApp --query "Stacks[0].StackStatus" --output text
